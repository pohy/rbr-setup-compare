...
async function scanRsfDataCars(
...
async function scanRsfDataSetups(
  root: FileSystemDirectoryHandle,
): Promise<ScannedSetup[]> {
  const rsfdata = await tryGetDirectory(root, 'rsfdata')
  if (!rsfdata) return []
...
export async function scanRbrDirectory(
  root: FileSystemDirectoryHandle,
): Promise<CarGroup[]> {
  ...
  console.log(`[rbr-scan] Scan complete — rsfCars: ${rsfCars.length}, rsfSetups: ${rsfSetups.length}, physics: ${physics.length}, total: ${all.length}`)
  if (all.length === 0) {
