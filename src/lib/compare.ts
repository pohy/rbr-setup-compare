...
export function parseFrontRearKey(key: string): { side: 'Front' | 'Rear'; counterpart: string } | null {
  const frontIdx = key.indexOf('Front')
  if (frontIdx !== -1) {
    return { side: 'Front', counterpart: key.slice(0, frontIdx) + 'Rear' + key.slice(frontIdx + 5) }
  }
...
export function computeRatio(front: number | string | null, rear: number | string | null): string | null {
  const f = typeof front === 'number' ? front : Number(front)
  const r = typeof rear === 'number' ? rear : Number(rear)
  if (front === null || rear === null || isNaN(f) || isNaN(r)) return null
  const sum = f + r
