...
import { isFileSystemAccessSupported, pickDirectory, checkPermission, verifyPermission, readFileHandle } from './fs-access.ts'
import { saveDirectoryHandle, loadDirectoryHandle, clearDirectoryHandle } from './directory-handle-store.ts'
import { scanRbrDirectory, type CarGroup, type ScannedSetup } from './rbr-scanner.ts'
...
export function useRbrDirectory() {
  ...
  const scan = useCallback(async (dirHandle: FileSystemDirectoryHandle) => {
    ...
    try {
      ...
      console.log(`[rbr-dir] Scan succeeded: ${groups.length} car groups, ${groups.reduce((n, g) => n + g.setups.length, 0)} total setups`)
      setCarGroups(groups)
      setHandle(dirHandle)
    } catch (e) {
  ...
  const loadSetups = useCallback(
    async (selected: ScannedSetup[]): Promise<CarSetup[]> => {
